...
- <span class="parameter">Show prediction button</span>: to display a button for the label predicted by the model, bound to the hotkey "P" (if Active Mode is on).
...
*Read about the concept of [Active learning](../conceptualizing/general.md#what-is-active-learning)*
...
To use BERT models in active learning, you must first compute the predictions on the dataset, in the [Evaluate tab](./model.md#evaluation) <span class="action primary">Compute statistics on current annotations</span>.
...
When applying multiple labels to the same element, the general layout is unchanged. To annotate elements, select one or more labels in the dropdown menu and <span class="action primary">![](../img/icons/validate.svg)</span> to save the annotation and move on to the next text input.
