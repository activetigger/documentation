...
- Clicking "Compute new projection" opens a modal with a form. You can choose the sets of features ([what are features](../theoretical-concepts/index.md#what-are-features)) to use, and parameters for the dimension reduction algorithm of your choice
    - **Select a feature**: You can choose as many sets features as you like. Choosing several sets of features, for each text input, the features will be concatenated. You can also create new features, opening the modal [Compute new features](../theoretical-concepts/index.md#compute-new-features).
    - Advanced parameters:
    ...
    - if you choose the **t-SNE algorithm**:
        - **perplexity**: XXX
            - **learning rate**: XXX
            - **init**: XXX
    - **Feature scaling**: this is a method normalizes the range of independant variables (i.e. each feature) ([should I scale my features?](../theoretical-concepts/index.md#should-i-scale-my-features)).
- Clicking "Parameters" opens a modal with the parameters of the current projection.
- The visualization panel displays each text input in a 2D space. The color of the node depends on the label, _of the absence thereof_. You will find several control buttons to navigate (![](../img/icons/visualisation-control/mouse.svg)), zoom in (![](../img/icons/visualisation-control/plus.svg)), zoom out (![](../img/icons/visualisation-control/minus.svg)) and recenter the graph (![](../img/icons/visualisation-control/center.svg)).
    - Clicking the frame button (![](../img/icons/visualisation-control/frame.svg)) allows you to select a subset of text inputs, and ![](../img/icons/visualisation-control/frame-off.svg) removes the selection.
    - If a frame exists, you can choose to **Lock the selection** ![](../img/icons/visualisation-control/lock.svg), going to the [Annotate page](./annotate.md) you will only see elements in this selection frame.
...
- The menu displays all topic models associated with the current project (and user ??? XXX). Each model can be deleted with the ![](../img/icons/delete.svg). Clicking "Compute new BERTopic" opens a modal with a form to create a new topic model with the following parameters :
    - Name: must be unique (with regard to? unique at all or unique per project ? unique per project per user? )
...
- Several interactions are made possible suc as: 
    - ![](../img/icons/visualisation-control/expand.svg) expands the visualization panel.
    - Parameters: Display the parameters of the current topic model in a modal.
    - Convert to scheme: Creates a [scheme](../theoretical-concepts/index.md#what-is-a-scheme) and assigns label using the topics generated. XXX Add NOTE REGARDING THE DANGERS OF DOING THAT
    - Export topics: Download (csv file) the topic table as displayed underneath.
    - Export topic per text: Download a mapping of each element id[^2] to a the topic index.
