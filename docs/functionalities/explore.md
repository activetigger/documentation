...
- <span class="action primary">Compute new projection</span>
    ...
    - <span class="parameter">Feature scaling</span>: this method normalizes the range of independant variables (i.e. each feature) ([should I scale my features?](../faq/faq.md#should-i-scale-my-features)).
...
- The visualization panel displays each text input in a 2D space. The color of the node depends on the label, _or the absence thereof_. 
    ...
    - Clicking a node displays the text input as well as previous annotations and the [Annotation Panel](./annotate.md#annotation-panel), much like in the [Annotation page](./annotate.md)
...
- <span class="action primary">Convert to scheme</span> to create a [scheme](../theoretical-concepts/glossary.md#schemes) and assign labels using the topics generated. <br/><span class="cwarning">⚠️ Note</span>: topic-generated labels should be used with caution and carefully reviewed, since they are not based on human annotation.
